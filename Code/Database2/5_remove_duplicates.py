...
Identifies duplicates based on config["duplicate_criteria"]:
  Route + Latitude + Longitude + Date + Total Observed

Rules:
  - Notes column excluded from criteria — rows with different notes
    can still be the same sighting
  - First occurrence of a duplicate group → kept
  - Subsequent occurrences → removed
  - Null values in criteria columns treated as equal
    (two rows both missing Latitude at same Route/Date = duplicate)
...
input_path   = os.path.join(output_folder, "database2_columns_selected.xlsx")
output_path  = os.path.join(output_folder, "database2_clean.xlsx")
