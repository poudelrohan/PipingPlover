...
Two checks run on every row:
  ...
  A) Location fields rule (config["location_fields"]):
       ...
       Removal reason: "Missing all location fields (Route, Latitude, Longitude)"

  B) Required fields rule (config["required_fields"]):
       - Date missing/empty → remove row
       Removal reason: "Missing required field: Date"

Output: database2_required_fields_ok.xlsx
"""
...
input_path   = os.path.join(output_folder, "database2_geo_validated.xlsx")
output_path  = os.path.join(output_folder, "database2_required_fields_ok.xlsx")
