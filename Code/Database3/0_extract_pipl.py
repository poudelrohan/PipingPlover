...
─────────────────────────────────────────
Reads the Species GPS sheet for the active year, extracts the PIPL count
from the free-text focal species column, normalizes column names to the
standard pipeline format, and outputs a flat file ready for Steps 1-6.

For years with a separate band resight sheet (2015+), merges band data
using Option A:
...
PIPL count extraction (priority order):
  ...
  7. Bare "PIPL" / "Piping Plover" → fallback to No. of individuals column,
     then assume 1
...
input_folder = os.path.normpath(os.path.join(script_dir, year_cfg["input_folder"]))
input_path   = os.path.join(input_folder, year_cfg["file"])
output_path  = os.path.join(output_folder, get_filename("extracted"))

cols = year_cfg["columns"]
...
# ══════════════════════════════════════════════════════════════════════════════
# Read GPS sheet (DATA SHEET 1 / Species GPS)
# ══════════════════════════════════════════════════════════════════════════════

df = pd.read_excel(
    input_path,
    sheet_name=year_cfg["sheet"],
    header=year_cfg["header_row"],
)
print(f"  Loaded {len(df)} rows from '{year_cfg['file']}' → sheet '{year_cfg['sheet']}'")
...
has_num   = year_cfg.get("has_num_individuals", False)
...
rename_map = {}
group_col = year_cfg.get("group_column")
...
# ══════════════════════════════════════════════════════════════════════════════
# Band sheet merge (Option A) — years with separate DS3
# ══════════════════════════════════════════════════════════════════════════════

band_cfg = year_cfg.get("band_sheet")
...
if band_cfg:
    ...
    # Build BandCombo by combining orientation + leg positions
    def build_band_combo(row):
        parts = []
        orientation = str(row.get(bcols["code_orientation"], "")) if pd.notna(row.get(bcols["code_orientation"])) else ""
    ...
    # Rename band columns to standard names
    band_rename = {}
    for std, src in [
    ...
    # ── Option A: Expand each DS1 flock row into banded + unbanded rows ────────
    expanded_rows = []
    ...
    for _, flock_row in df.iterrows():
        ...
        # Find matching band rows: same date + route + group
        matched_bands = bands[
        ...
        if n_banded == 0:
            # No banded birds — keep original row as-is
            ...
            for _, band_row in matched_bands.iterrows():
                ...
                # Append band comments to main comments if present
                ...
                if pd.notna(band_comment) and str(band_comment).strip():
                    ...
                    if existing:
                        row_data["Comments"] = f"{existing} | Band: {band_comment}"
                    else:
                        row_data["Comments"] = f"Band: {band_comment}"
                expanded_rows.append(row_data)
    ...
    print(f"\n  After Option A expansion: {len(df)} rows "
          f"(was {extracted} flocks, added {len(df) - extracted} band rows)")

    # Check for unmatched band rows
    matched_keys = set()
    for _, flock_row in pd.DataFrame(expanded_rows).iterrows():
        pass  # already processed above

else:
    # No band sheet — for years where band info is in the GPS sheet itself
    pass
