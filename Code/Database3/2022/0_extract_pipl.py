"""
Step 0: Extract PIPL Data — 2022
──────────────────────────────────
Reads the 2022 Band Review file (wide format, one row per route) and melts
...
sheet — it lives in the "All Species" sheet of Winter Birds 2022.xlsx
and is joined on the route name (col 0 = "Transect ").

Band data source priority per (route, point):
  1. Biologist correction  — Biologist_Band_Review_Completed_2022.xlsx col H
...
2022-specific handling (mirrors 2020):
  ...
  ⑥ GPS fallback: 2019, 2020, 2022–2024 clean files (skip 2022 itself).
  ...
  ⑧ One Band Review route uses a different name in All Species
    ("Biscayne National Park - ..." vs "Biscayne National Park/ Convoy Point to ...")
    — resolved via config["route_aliases"] in build_metadata_lookup.

Output: db3_2022_extracted.xlsx  (in Output/2022/)
"""
...
def parse_coord(val):
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return None
    try:
        # Strip degree symbol and surrounding whitespace — some 2022 rows
...
def build_biologist_corrections():
    ...
    review_path_bio = (
        ...
        / "Biologist_Band_Review_Completed_2022.xlsx"
    )
...
print(f"\n{'='*60}")
print(f"Step 0 — 2022 PIPL extraction")
    ...
    for n in points:
        ...
        if lat is None or lon is None:
            ...
            if fb_key in gps_fallback:
                ...
                gps_note = f"GPS borrowed from {fb_yr} — point {n} had no coordinates in 2022"
