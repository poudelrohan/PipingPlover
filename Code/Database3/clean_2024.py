"""
Clean 2024 Winter Bird Survey — filter to PIPL rows and columns only
─────────────────────────────────────────────────────────────────────
Same two-sheet structure as 2023.
  "All Species"        — route totals (metadata + PIPL count)
  "Focal Observations" — per-group GPS detail (19 points, MOST IMPORTANT)

Sheets skipped: Directions, Sheet14, Summary Data, County Summary,
  "For sites with >19 points" (no PIPL data).

Key differences vs 2020–2022 (same as 2023):
  - All Species col 0: "Transect (if yours is missing, email Beth Forys: ...)"
    (handled dynamically via all_sp.columns[0])
  - All Species has 11 metadata cols (0–10), including "If you did not survey"
...
Known issues (do NOT fix here — log for pipeline):
  ...
  - AllSp=43 PIPL rows vs Focal=36 PIPL rows (7 routes have totals but no GPS groups)

Output: Databases/Database3Clean/Winter Birds 2024 Clean.xlsx
"""
...
input_path  = os.path.normpath(os.path.join(script_dir, "../../Databases/Database3/Winter Birds 2024.xlsx"))
output_path = os.path.normpath(os.path.join(script_dir, "../../Databases/Database3Clean/Winter Birds 2024 Clean.xlsx"))
...
# 2024 has 11 metadata cols (0–10): same layout as 2023
