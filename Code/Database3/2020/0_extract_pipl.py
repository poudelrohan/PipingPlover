"""
Step 0: Extract PIPL Data — 2020
──────────────────────────────────
Reads the 2020 Band Review file (wide format, one row per route) and melts
...
sheet for 2020 — it lives in the "All Species" sheet of Winter Birds 2020.xlsx
and is joined on the route name (col 0 = "Transect ").

Band data source priority per (route, point):
  1. Biologist correction  — Biologist_Band_Review_Completed_2020.xlsx col H
...
Two extensions to the 2019 _resolve_band_text logic (per biologist feedback):
  • "good" confirmations broadened: any text containing "good" (no "1)" / "/")
    is treated as confirmation → use our interpretation.
  • "treat as unbanded" directive honored in EITHER `Our Notes (read only)` or
    `Your Correction` column → forces unbanded (0 banded, all PIPL as remainder).

2020-specific handling:
  ① Column names are short clean strings — regex matches on
     'Latitude/Longitude/Number/PIPL ... point N'.
  ...
  ⑥ GPS fallback: 2019, 2021–2024 clean files (skip 2020 itself).

Output: db3_2020_extracted.xlsx  (in Output/2020/)
"""
...
def _resolve_band_text(notes: str, correction: str, interpretation: str):
    ...
    # 4) Confirmation phrases (broadened to match any "good")
    if "good" in t and "1)" not in t and "/" not in t:
...
def build_biologist_corrections():
    ...
    review_path_bio = (
        ...
        / "Biologist_Band_Review_Completed_2020.xlsx"
    )
...
print(f"\n{'='*60}")
print(f"Step 0 — 2020 PIPL extraction")
    ...
    for n in points:
        ...
        if lat is None or lon is None:
            ...
            if fb_key in gps_fallback:
                ...
                gps_note = f"GPS borrowed from {fb_yr} — point {n} had no coordinates in 2020"
