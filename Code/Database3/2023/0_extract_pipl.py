"""
Step 0: Extract PIPL Data — 2023
──────────────────────────────────
Reads the 2023 Band Review file (wide format, one row per route) and melts
...
sheet — it lives in the "All Species" sheet of Winter Birds 2023.xlsx
and is joined on the route name (col 0 = "Transect ").

Band data source priority per (route, point):
  1. Biologist correction  — Biologist_Band_Review_Completed_2023.xlsx col H
...
2023-specific handling (mirrors 2020):
  ...
  ⑥ GPS fallback: 2019, 2020, 2023–2024 clean files (skip 2023 itself).
  ...
  ⑧ All Species column layout shifted in 2023 — Transect header expanded
    and a new "If you did not survey" column inserted. Handled by updating
    cols["route"] in the 2023 config; all other columns matched by name.

Output: db3_2023_extracted.xlsx  (in Output/2023/)
"""
...
def parse_coord(val):
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return None
    try:
        # Strip degree symbol and surrounding whitespace — some 2023 rows
...
def build_biologist_corrections():
    ...
    review_path_bio = (
        ...
        / "Biologist_Band_Review_Completed_2023.xlsx"
    )
...
print(f"\n{'='*60}")
print(f"Step 0 — 2023 PIPL extraction")
    ...
    for n in points:
        ...
        if lat is None or lon is None:
            ...
            if fb_key in gps_fallback:
                ...
                gps_note = f"GPS borrowed from {fb_yr} — point {n} had no coordinates in 2023"
