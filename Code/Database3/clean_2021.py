"""
Clean 2021 Winter Bird Survey — filter to PIPL rows and columns only
─────────────────────────────────────────────────────────────────────
Same structure as 2020: two sheets kept.
...
"Directions", "Summary Data", "transects with no data", "Sheet8" — skipped.

Known issues (do NOT fix here — log for pipeline):
  - "Totals" row appears in All Species with PIPL=312 — summary row, not a transect
  - Little Talbot Island SP: AllSp=13, Focal=15 (+2 in focal)
  - Flag Island: AllSp=5, Focal=6 (+1 in focal)
  - Crandon Park Beach: AllSp=26, Focal=0 (route total filled, no GPS groups)
  - Palm Beach (Monceau Court) to Breakers Resort: AllSp=7, Focal=2 (two focal rows; count mismatch)
  - Outback Key: AllSp=31, Focal=0 (no GPS groups)

Output: Databases/Database3Clean/Winter Birds 2021 Clean.xlsx
"""
...
input_path  = os.path.normpath(os.path.join(script_dir, "../../Databases/Database3/Winter Birds 2021.xlsx"))
output_path = os.path.normpath(os.path.join(script_dir, "../../Databases/Database3Clean/Winter Birds 2021 Clean.xlsx"))
