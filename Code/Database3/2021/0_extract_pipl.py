"""
Step 0: Extract PIPL Data — 2021
──────────────────────────────────
Reads the 2021 Band Review file (wide format, one row per route) and melts
...
sheet — it lives in the "All Species" sheet of Winter Birds 2021.xlsx
and is joined on the route name (col 0 = "Transect ").

Band data source priority per (route, point):
  1. Biologist correction  — Biologist_Band_Review_Completed_2021.xlsx col H
...
Same _resolve_band_text logic as 2020:
...
2021-specific handling (mirrors 2020):
  ...
  ⑥ GPS fallback: 2019, 2020, 2022–2024 clean files (skip 2021 itself).
...
Output: db3_2021_extracted.xlsx  (in Output/2021/)
"""
...
def build_biologist_corrections():
    ...
    review_path_bio = (
        ...
        / "Biologist_Band_Review_Completed_2021.xlsx"
    )
...
print(f"\n{'='*60}")
print(f"Step 0 — 2021 PIPL extraction")
    ...
    for n in points:
        ...
        if lat is None or lon is None:
            ...
            if fb_key in gps_fallback:
                ...
                gps_note = f"GPS borrowed from {fb_yr} — point {n} had no coordinates in 2021"
