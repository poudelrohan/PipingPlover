...
if "_geo_warning" in clean_df.columns:
    ...
    for _, row in warned.iterrows():
        geo_warnings.append({"unique_id": row.get("unique_id"), "warning": row["_geo_warning"]})
...
if "_geo_correction" in clean_df.columns:
    ...
    for _, row in corrected.iterrows():
        geo_corrections.append({"unique_id": row.get("unique_id"), "correction": row["_geo_correction"]})
...
if len(removed_df) > 0:
    ...
    final_removed_cols = [c for c in front_cols if c in removed_sheet.columns] + clean_cols
...
for _, row in clean_df.iterrows():
    ...
    if 0 < len(missing) < len(loc_fields):
        partial_warnings.append({"unique_id": row.get("unique_id"), "missing_fields": ", ".join(missing)})
